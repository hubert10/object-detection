...
class MeanAveragePrecisionCallback(Callback):
    ...
    def _load_weights_for_model(self):
        ...
        self._verbose_print(
            "Loaded weights for the inference model (last checkpoint of the train model): {0}".format(
                last_weights_path
            )
        )
    ...
    def _calculate_mean_average_precision(self):
        ...
        for image_id in self.dataset_image_ids[: self.dataset_limit]:
            image, image_meta, gt_class_id, gt_bbox, gt_mask = load_image_gt(
                self.dataset, self.inference_model.config, image_id, use_mini_mask=False
            )
            molded_images = np.expand_dims(
                mold_image(image, self.inference_model.config), 0
            )
