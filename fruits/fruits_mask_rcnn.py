...
# train set
dataset_dir = "fruits/renamed_to_numbers"
...
DEFAULT_LOGS_DIR = os.path.join(ROOT_DIR, "fruits/logs")

###############

# define the model
model = MaskRCNN(mode="training", model_dir="fruits/logs", config=config)
# load weights (mscoco) and exclude the output layers
model.load_weights(
    "fruits/weights/mask_rcnn_coco.h5",
    by_name=True,
...
model.train(
    train_set, test_set, learning_rate=config.LEARNING_RATE, epochs=20, layers="heads"
)
#########################################
...
# load model weights
model.load_weights("fruits/logs/mask_rcnn_fruits_cfg_0020.h5", by_name=True)
...
fruit_img = skimage.io.imread("fruits/samples/fruit-bowl.jpg")
...
for box in detected["rois"]:
    ...
    # create the shape
    ax.annotate(
        class_names[detected_class_id - 1],
        (x1, y1),
        color="black",
        weight="bold",
        fontsize=10,
